...
Expr can be configured to do more things. For example, with [AllowUndefinedVariables](https://pkg.go.dev/github.com/oarkflow/expr#AllowUndefinedVariables) or [AsBool](https://pkg.go.dev/github.com/oarkflow/expr#AsBool) to expect the boolean result from the expression.
...
Also, Expr supports functions configured via [`expr.Function(name, fn[, ...types])`](https://pkg.go.dev/github.com/oarkflow/expr#Function) option.
